...
import { StoryLog } from './Layout/StoryLog';
import { PlayerControls } from './Layout/PlayerControls';
import { RightPanel } from './Layout/RightPanel';
import { WindowManager } from './Layout/WindowManager';
import { Button, Label, Input, TextArea } from './ui/Button';
...
export default function App() {
  ...
  const restartGame = () => {
      setConfirmModal({
          ...
          message: "警告：这将完全清空当前游戏的所有进度，删除自动存档，并恢复到【系统初始设置】。您当前自定义的全局设置（除API Key外）也将丢失。此操作不可撤销。",
          onConfirm: () => {
              game.resetGame();
          }
      });
  ...
  const handleTogglePause = () => {
      game.updateState(s => ({...s, round: {...s.round, isPaused: !s.round.isPaused}}));
  };
          ...
          <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center">
              <div className={`bg-slate-900 border border-slate-700 rounded-lg p-6 w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200 flex flex-col ${isImportMode ? 'max-w-2xl max-h-[85vh]' : 'max-w-md'}`}>
                                          ...
                                          {importSettings.keepMemory && (
                                              <div className="flex items-center gap-2 text-xs">
                                                  ...
                                                  <Input 
                                                      ...
                                                      onChange={e => setImportSettings({...importSettings, memoryRounds: Math.max(1, parseInt(e.target.value) || 20)})}
                                                  />
                                              </div>
                                          )}
                                      </div>
                  ...
                  <div className="flex justify-end gap-3 mt-6 shrink-0">
                      <Button variant="secondary" onClick={() => game.setSaveLoadModal({ ...game.saveLoadModal, isOpen: false })}>取消</Button>
